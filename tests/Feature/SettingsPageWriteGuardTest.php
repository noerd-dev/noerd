...
/*
 ...
 | unfiltered keys — a full privilege escalation on a sensitive model (stood
 | in for by a zz fixture model). Two independent defenses are asserted here, so
...
it('rejects a client update to the settings model map', function (): void {
    expect(fn() => Livewire::test('zz-guard-settings-page')->set('settingsModels', ['detailData' => ZzSettingsProfile::class]))
...
it('writes nothing for a bound property the settings YAML does not declare', function (): void {
    ...
    $target = ZzSettingsProfile::create(['tenant_id' => $this->tenantId, 'key' => 'USER', 'name' => 'Member']);

    $component = Livewire::test('zz-guard-settings-page');
    // Server-side repoint (simulates any path that sets the map after mount).
    $component->instance()->settingsModels = ['relationTitles' => ZzSettingsProfile::class];

    $component->set('relationTitles', ['key' => 'ADMIN', 'name' => 'pwned'])
        ->call('store');

    expect($target->refresh()->key)->toBe('USER')
        ->and($target->name)->toBe('Member')
        ->and(ZzSettingsProfile::where('tenant_id', $this->tenantId)->count())->toBe(1);
});
