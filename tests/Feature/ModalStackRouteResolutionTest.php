...
it('resolves a route name to its component and rewrites the url', function (): void {
    registerTestLivewireRoute('zz-modal-record/{modelId}', 'noerd::theme-test', 'zz.modal.record');

    [$component, $modals] = openModal(['route' => 'zz.modal.record', 'arguments' => ['modelId' => 5]]);

    expect($modals[0]['componentName'])->toBe('noerd::theme-test')
...
it('opens a component name as-is and leaves the url alone', function (): void {
    [$component, $modals] = openModal([
        'modalComponent' => 'noerd::theme-test',
        'arguments' => ['modelId' => 5],
    ]);
...
it('prefers the route over the fallback component when both are given', function (): void {
    registerTestLivewireRoute('zz-modal-wins/{modelId}', 'noerd::positions-theme-test', 'zz.modal.wins');
    ...
    expect($modals[0]['componentName'])->toBe('noerd::positions-theme-test');
});
...
it('carries the new sentinel in the url for a create modal', function (): void {
    registerTestLivewireRoute('zz-modal-new/{modelId}', 'noerd::theme-test', 'zz.modal.new');
...
it('resolves the route but keeps the url when rewriteUrl is false', function (): void {
    registerTestLivewireRoute('zz-modal-no-rewrite/{modelId}', 'noerd::theme-test', 'zz.modal.no.rewrite');
...
it('suppresses the url rewrite for an argument the route cannot express', function (): void {
    registerTestLivewireRoute('zz-modal-filtered', 'noerd::theme-test', 'zz.modal.filtered');
...
it('still rewrites the url for chrome-only arguments', function (): void {
    registerTestLivewireRoute('zz-modal-chrome/{modelId}', 'noerd::theme-test', 'zz.modal.chrome');
...
it('restores the url only once the routed modal itself is closed', function (): void {
    registerTestLivewireRoute('zz-modal-stack/{modelId}', 'noerd::theme-test', 'zz.modal.stack');

    $component = Livewire::test('noerd-modal::noerd-modal')
        ->dispatch('noerdModal', route: 'zz.modal.stack', arguments: ['modelId' => 5])
        ->dispatch('noerdModal', modalComponent: 'noerd::theme-test', arguments: ['sub' => true]);
