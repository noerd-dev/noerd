...
@php
    $spacerClass = app(\Noerd\Services\DetailViewRegistry::class)
        ->get($field['view'] ?? (($field['compact'] ?? false) ? 'compact' : 'default'))
        ->spacerClass;
@endphp

{{-- Layout spacer: renders an empty cell the same height as a standard input field (label +
     input in the default layout, just the input height in the denser views), so it reads as a
